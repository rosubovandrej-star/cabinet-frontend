...
**Action:** При анимации значений с помощью хуков типа `useAnimatedNumber`, извлекайте вызов хука и конкретный DOM-узел, отображающий анимированное значение, в выделенный микрокомпонент.## 2026-03-17 - Изоляция интервалов времени (State Colocation для таймеров)
**Learning:** В крупных компонентах, таких как `Dashboard`, наличие локального состояния для обратного отсчета (например, обновление каждую секунду через `setInterval`) приводит к перерисовке всего дерева компонентов каждую секунду.
**Action:** Состояние таймеров и интервалов, которые обновляют UI каждую секунду, следует инкапсулировать в выделенные микрокомпоненты. При передаче пропсов используйте абсолютные отметки времени (timestamp), а не относительные счетчики.
